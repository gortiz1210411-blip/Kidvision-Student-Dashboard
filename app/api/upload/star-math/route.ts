...
import { validateRows, StarMathRowSchema, resolveKidvisionIds } from "@/utils/validators";
...
export async function POST(req: Request) {
  try {
    ...
    const insertPayload = resolvedRows.map((row) => ({
      ...
      assessment_type: "STAR_MATH",
      subject: "math",
      scale_score: Number(row.scale_score ?? null),
      percentile_rank: Number(row.percentile_rank ?? null),
      level: row.level ?? null,
      test_name: row.test_name || "STAR Math",
